...
type Article = { slug: string; title: string; desc: string; tag: string };
...
const categories: { label: string; articles: Article[] }[] = [
  {
    label: 'Crime Trends',
    articles: [
  ...
  },
  {
    ...
    articles: [
      ...
      { slug: 'crime-and-politics', title: 'Do Red States or Blue States Have More Crime?', desc: 'Data-driven analysis: both sides cherry-pick stats. The real variable is urbanization, not politics. Top 10 most violent states include both colors.', tag: 'DEEP DIVE' },
      ...
      { slug: 'safest-places-to-live', title: 'The Safest Places to Live in America: A Data-Driven Guide', desc: 'Top 50 safest cities, 25 safest small towns, and safest states. Find the safest places based on FBI violent crime data.', tag: 'DEEP DIVE' },
      { slug: 'crime-rates-by-state-2026', title: 'Crime Rates by State 2026 — All 50 States Ranked', desc: 'Every US state ranked by violent crime rate with regional breakdowns, murder rankings, and year-over-year trends.', tag: 'DEEP DIVE' },
      { slug: 'safest-states-2026', title: 'Safest States to Live In 2026', desc: 'The 25 safest US states ranked by violent crime rate. New England dominates with rates 60-70% below average.', tag: 'DEEP DIVE' },
      { slug: 'violent-crime-statistics-2026', title: 'Violent Crime Statistics 2026 — The Complete Picture', desc: 'Deep dive into murder, assault, robbery, and rape statistics with 45 years of historical trends and category breakdowns.', tag: 'DEEP DIVE' },
      { slug: 'most-dangerous-cities-2026', title: 'Most Dangerous Cities in America 2026', desc: 'The 25 most dangerous US cities ranked by violent crime rate using FBI data. See which cities top the list.', tag: 'DEEP DIVE' },
...
function ArticleCard({ article, isSlug }: { article: Article; isSlug: boolean }) {
  const href = isSlug ? `/analysis/${article.slug}` : (article as unknown as { href: string }).href;
  return (
    <Link href={href} className="block bg-white rounded-xl shadow-sm border p-4 hover:shadow-md transition group">
      <div className="flex items-start gap-3">
        <span className={`text-xs font-bold px-2 py-1 rounded shrink-0 ${tagColors[article.tag] || 'bg-gray-100'}`}>{article.tag}</span>
        <div>
          <h3 className="font-heading text-base font-bold group-hover:text-[#1e3a5f] transition leading-snug">{article.title}</h3>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <p className="text-gray-600 mb-8">
        ...
        <span className="text-gray-400 text-sm">({totalArticles} articles)</span>
      </p>
        ...
        {categories.map(cat => (
          <section key={cat.label}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="bg-gray-100 text-gray-500 text-xs font-bold px-2 py-0.5 rounded-full">{cat.articles.length}</span>
            </div>
            <div className="grid md:grid-cols-2 gap-3">
              {cat.articles.map(a => (
                <ArticleCard key={a.slug} article={a} isSlug={true} />
              ))}
            </div>
          </section>
        ))}
      ...
      <h2 className="font-heading text-2xl font-bold mt-14 mb-2">Unique Data Explorations</h2>
      <p className="text-gray-600 mb-6 text-sm">Interactive tools and unique datasets that go beyond standard crime statistics.</p>
      <div className="grid md:grid-cols-2 gap-3">
        {specialPages.map(a => (
          <Link key={a.href} href={a.href} className="block bg-white rounded-xl shadow-sm border p-4 hover:shadow-md transition group">
            <div className="flex items-start gap-3">
              <span className={`text-xs font-bold px-2 py-1 rounded shrink-0 ${tagColors[a.tag] || 'bg-gray-100'}`}>{a.tag}</span>
              <div>
                <h3 className="font-heading text-base font-bold group-hover:text-[#1e3a5f] transition leading-snug">{a.title}</h3>
